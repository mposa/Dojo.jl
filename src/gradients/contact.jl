function get_contact_gradients_with_inertia(mechanism::Mechanism{T,Nn,Ne,Nb,Ni}) where {T,Nn,Ne,Nb,Ni}
	...
	index_inertia = [index_col[body.id][1:7] for body in mechanism.bodies]
	...
	for (i, body) in enumerate(mechanism.bodies)
		...
		# jacobian_control[12*(i-1) .+ (1:3),:] += Dojo.linear_integrator_jacobian_velocity(x2, v25, timestep) * data_jacobian[index_row[id][1:3], vcat(index_control...)]
		# jacobian_control[12*(i-1) .+ (7:9),:] += Dojo.LVᵀmat(q3)' * rotational_integrator_jacobian_velocity(q2, ω25, timestep) * data_jacobian[index_row[id][4:6], vcat(index_control...)]
		...
		jacobian_inertia[12*(i-1) .+ (1:3),:] += Dojo.linear_integrator_jacobian_velocity(x2, v25, timestep) * data_jacobian[index_row[id][1:3], vcat(index_inertia...)]
		jacobian_inertia[12*(i-1) .+ (7:9),:] += Dojo.LVᵀmat(q3)' * Dojo.rotational_integrator_jacobian_velocity(q2, ω25, timestep) * data_jacobian[index_row[id][4:6], vcat(index_inertia...)]
	end

	return jacobian_state, jacobian_contact, jacobian_inertia
end